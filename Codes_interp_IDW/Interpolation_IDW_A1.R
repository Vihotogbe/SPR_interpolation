...
## Precipitation
powers <- 5
idw_interp_pr_A1_10 <- foreach(i = 1:length(powers), .combine = rbind,
...
save(idw_interp_pr_A1_10, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_pr_A1_10.RData")


## Minimum temperature
powers <- 5
idw_interp_tasmin_A1_10 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
...
# Save the results in appropriate folder
save(idw_interp_tasmin_A1_10, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmin_A1_10.RData")


## Maximum temperature
powers <- 5
idw_interp_tasmax_A1_10 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
...
# Save the results in appropriate folder
save(idw_interp_tasmax_A1_10, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmax_A1_10.RData")
...
## Precipitation
powers <- 5
idw_interp_pr_A1_30 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_pr <- idw_interp(data_train = data_pr_train, coord_train = coord_train,
                              coord_test = coord_test, power = powers)
  interp_idw_pr <- log(1 + exp(interp_idw_pr))
...
# Save the results in appropriate folder
save(idw_interp_pr_A1_30, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_pr_A1_30.RData")


## Minimum temperature
powers <- 5
idw_interp_tasmin_A1_30 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_tasmin <- idw_interp(data_train = data_tasmin_train, coord_train = coord_train,
                                  coord_test = coord_test, power = powers)
  return(interp_idw_tasmin)
...
# Save the results in appropriate folder
save(idw_interp_tasmin_A1_30, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmin_A1_30.RData")


## Maximum temperature
powers <- 5
idw_interp_tasmax_A1_30 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_tasmax <- idw_interp(data_train = data_tasmax_train, coord_train = coord_train,
                                  coord_test = coord_test, power = powers)
  return(interp_idw_tasmax)
...
# Save the results in appropriate folder
save(idw_interp_tasmax_A1_30, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmax_A1_30.RData")
...
## Precipitation
powers <- 4
idw_interp_pr_A1_50 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_pr <- idw_interp(data_train = data_pr_train, coord_train = coord_train,
                              coord_test = coord_test, power = powers)
  interp_idw_pr <- log(1 + exp(interp_idw_pr))
...
# Save the results in appropriate folder
save(idw_interp_pr_A1_50, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_pr_A1_50.RData")


## Minimum temperature
powers <- 4
idw_interp_tasmin_A1_50 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_tasmin <- idw_interp(data_train = data_tasmin_train, coord_train = coord_train,
                                  coord_test = coord_test, power = powers)
  return(interp_idw_tasmin)
...
# Save the results in appropriate folder
save(idw_interp_tasmin_A1_50, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmin_A1_50.RData")


## Maximum temperature
powers <- 4
idw_interp_tasmax_A1_50 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_tasmax <- idw_interp(data_train = data_tasmax_train, coord_train = coord_train,
                                  coord_test = coord_test, power = powers)
  return(interp_idw_tasmax)
...
# Save the results in appropriate folder
save(idw_interp_tasmax_A1_50, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmax_A1_50.RData")
...
## Precipitation
powers <- 4
idw_interp_pr_A1_70 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_pr <- idw_interp(data_train = data_pr_train, coord_train = coord_train,
                              coord_test = coord_test, power = powers)
  interp_idw_pr <- log(1 + exp(interp_idw_pr))
...
# Save the results in appropriate folder
save(idw_interp_pr_A1_70, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_pr_A1_70.RData")


## Minimum temperature
powers <- 3
idw_interp_tasmin_A1_70 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_tasmin <- idw_interp(data_train = data_tasmin_train, coord_train = coord_train,
                                  coord_test = coord_test, power = powers)
  return(interp_idw_tasmin)
...
# Save the results in appropriate folder
save(idw_interp_tasmin_A1_70, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmin_A1_70.RData")


## Maximum temperature
powers <- 3
idw_interp_tasmax_A1_70 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_tasmax <- idw_interp(data_train = data_tasmax_train, coord_train = coord_train,
                                  coord_test = coord_test, power = powers)
  return(interp_idw_tasmax)
...
# Save the results in appropriate folder
save(idw_interp_tasmax_A1_70, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmax_A1_70.RData")
...
## Precipitation
powers <- 3
idw_interp_pr_A1_90 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_pr <- idw_interp(data_train = data_pr_train, coord_train = coord_train,
                              coord_test = coord_test, power = powers)
  interp_idw_pr <- log(1 + exp(interp_idw_pr))
...
# Save the results in appropriate folder
save(idw_interp_pr_A1_90, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_pr_A1_90.RData")


## Minimum temperature
powers <- 3
idw_interp_tasmin_A1_90 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_tasmin <- idw_interp(data_train = data_tasmin_train, coord_train = coord_train,
                                  coord_test = coord_test, power = powers)
  return(interp_idw_tasmin)
...
# Save the results in appropriate folder
save(idw_interp_tasmin_A1_90, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmin_A1_90.RData")


## Maximum temperature
powers <- 3
idw_interp_tasmax_A1_90 <- foreach(i = 1:length(powers), .combine = rbind,
                          .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  interp_idw_tasmax <- idw_interp(data_train = data_tasmax_train, coord_train = coord_train,
                                  coord_test = coord_test, power = powers)
  return(interp_idw_tasmax)
...
# Save the results in appropriate folder
save(idw_interp_tasmax_A1_90, file = "Projet_1/Interpolation_finale/Simulations_IDW/Region_A1/idw_interp_tasmax_A1_90.RData")
