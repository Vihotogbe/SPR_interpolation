...
## Precipitation
model <- "Sph"
ok_interp_pr_B3_10 <- foreach(i = 1:length(model), .combine = rbind,
...
save(ok_interp_pr_B3_10, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_pr_B3_10.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_B3_10 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
...
# Save the results in appropriate folder
save(ok_interp_tasmin_B3_10, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmin_B3_10.RData")


## Maximum temperature
model <- "Sph"
ok_interp_tasmax_B3_10 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
...
# Save the results in appropriate folder
save(ok_interp_tasmax_B3_10, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmax_B3_10.RData")
...
## Precipitation
model <- "Sph"
ok_interp_pr_B3_30 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_pr <- ok_interp(data_train = data_pr_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_pr_B3_30, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_pr_B3_30.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_B3_30 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmin <- ok_interp(data_train = data_tasmin_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmin_B3_30, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmin_B3_30.RData")


## Maximum temperature
model <- "Sph"
ok_interp_tasmax_B3_30 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmax <- ok_interp(data_train = data_tasmax_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmax_B3_30, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmax_B3_30.RData")
...
## Precipitation
model <- "Sph"
ok_interp_pr_B3_50 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_pr <- ok_interp(data_train = data_pr_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_pr_B3_50, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_pr_B3_50.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_B3_50 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmin <- ok_interp(data_train = data_tasmin_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmin_B3_50, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmin_B3_50.RData")


## Maximum temperature
model <- "Sph"
ok_interp_tasmax_B3_50 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmax <- ok_interp(data_train = data_tasmax_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmax_B3_50, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmax_B3_50.RData")
...
## Precipitation
model <- "Sph"
ok_interp_pr_B3_70 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_pr <- ok_interp(data_train = data_pr_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_pr_B3_70, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_pr_B3_70.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_B3_70 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmin <- ok_interp(data_train = data_tasmin_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmin_B3_70, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmin_B3_70.RData")


## Maximum temperature
model <- "Exp"
ok_interp_tasmax_B3_70 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmax <- ok_interp(data_train = data_tasmax_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmax_B3_70, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmax_B3_70.RData")
...
## Precipitation
model <- "Exp"
ok_interp_pr_B3_90 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_pr <- ok_interp(data_train = data_pr_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_pr_B3_90, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_pr_B3_90.RData")


## Minimum temperature
model <- "Exp"
ok_interp_tasmin_B3_90 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmin <- ok_interp(data_train = data_tasmin_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmin_B3_90, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmin_B3_90.RData")


## Maximum temperature
model <- "Exp"
ok_interp_tasmax_B3_90 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmax <- ok_interp(data_train = data_tasmax_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmax_B3_90, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_B3/ok_interp_tasmax_B3_90.RData")
