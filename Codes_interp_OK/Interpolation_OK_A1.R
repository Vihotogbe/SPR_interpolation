...
# ### REMOVE AFTER
# # Calculate anomalies
# anomalies <- function(data) {
#   ndays <- 365
#   nyears <- 10
#   days <- seq(1:ndays)
#   data.days <- matrix(0, nrow = ndays, ncol = ncol(data))
#   for (i in 1:nyears) {
#     data_year <- data[days, ] # Select the data for the year
#     data.days <- data.days + data_year # Sum the data for each year
#     days <- days + ndays
#   }
#   data.mean.daily <- data.days / nyears
#   data.mean.daily.mat <- do.call(rbind, replicate(nyears, data.mean.daily, simplify = FALSE))
#   data.anomalies <- data - data.mean.daily.mat
#   my_list <- list("data.mean.daily" = data.mean.daily, "data.anomalies" = data.anomalies)
#   return(my_list)
# }

# data_anomalies_tasmin <- anomalies(data_tasmin_obs)$data.anomalies
# data_tasmin_obs <- data_anomalies_tasmin
# data_anomalies_tasmax <- anomalies(data_tasmax_obs)$data.anomalies
# data_tasmax_obs <- data_anomalies_tasmax
...
## Precipitation
model <- "Sph"
ok_interp_pr_A1_10 <- foreach(i = 1:length(model), .combine = rbind,
...
save(ok_interp_pr_A1_10, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_pr_A1_10.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_A1_10 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
...
# Save the results in appropriate folder
save(ok_interp_tasmin_A1_10, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmin_A1_10.RData")


## Maximum temperature
model <- "Exp"
ok_interp_tasmax_A1_10 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
...
# Save the results in appropriate folder
save(ok_interp_tasmax_A1_10, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmax_A1_10.RData")
...
## Precipitation
model <- "Sph"
ok_interp_pr_A1_30 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_pr <- ok_interp(data_train = data_pr_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_pr_A1_30, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_pr_A1_30.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_A1_30 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmin <- ok_interp(data_train = data_tasmin_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmin_A1_30, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmin_A1_30.RData")


## Maximum temperature
model <- "Exp"
ok_interp_tasmax_A1_30 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmax <- ok_interp(data_train = data_tasmax_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmax_A1_30, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmax_A1_30.RData")
...
## Precipitation
model <- "Sph"
ok_interp_pr_A1_50 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_pr <- ok_interp(data_train = data_pr_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_pr_A1_50, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_pr_A1_50.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_A1_50 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmin <- ok_interp(data_train = data_tasmin_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmin_A1_50, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmin_A1_50.RData")


## Maximum temperature
model <- "Sph"
ok_interp_tasmax_A1_50 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmax <- ok_interp(data_train = data_tasmax_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmax_A1_50, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmax_A1_50.RData")
...
## Precipitation
model <- "Sph"
ok_interp_pr_A1_70 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_pr <- ok_interp(data_train = data_pr_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_pr_A1_70, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_pr_A1_70.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_A1_70 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmin <- ok_interp(data_train = data_tasmin_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmin_A1_70, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmin_A1_70.RData")


## Maximum temperature
model <- "Sph"
ok_interp_tasmax_A1_70 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmax <- ok_interp(data_train = data_tasmax_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmax_A1_70, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmax_A1_70.RData")
...
## Precipitation
model <- "Sph"
ok_interp_pr_A1_90 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_pr <- ok_interp(data_train = data_pr_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_pr_A1_90, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_pr_A1_90.RData")


## Minimum temperature
model <- "Sph"
ok_interp_tasmin_A1_90 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmin <- ok_interp(data_train = data_tasmin_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmin_A1_90, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmin_A1_90.RData")


## Maximum temperature
model <- "Sph"
ok_interp_tasmax_A1_90 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %dopar% {
  # Perform IDW for each model
  model <- model
  interp_ok_tasmax <- ok_interp(data_train = data_tasmax_train, coord_train = coord_train,
...
# Save the results in appropriate folder
save(ok_interp_tasmax_A1_90, file = "Projet_1/Interpolation_finale/Simulations_OK/Region_A1/ok_interp_tasmax_A1_90.RData")
