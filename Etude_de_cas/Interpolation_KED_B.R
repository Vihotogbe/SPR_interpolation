...
library(doParallel)
library(foreach)
cl <- makeCluster(25) # 15 cores
registerDoParallel(cl)
###############################################################

#######################################################################################
################################## 99.9% of missing data ################################
...
## Precipitation
model <- "Sph"
ked_interp_pr_B1 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %do% {
...
save(ked_interp_pr_B1, file = "Projet_1/Interpolation_finale/Etude_de_cas/ked_interp_pr_B1.RData")


## Minimum temperature
model <- "Sph"
ked_interp_tasmin_B1 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %do% {
  # Perform KED for each model
...
# Save the results in appropriate folder
save(ked_interp_tasmin_B1, file = "Projet_1/Interpolation_finale/Etude_de_cas/ked_interp_tasmin_B1.RData")

## Maximum temperature
model <- "Sph"
ked_interp_tasmax_B1 <- foreach(i = 1:length(model), .combine = rbind,
                            .packages = c("gstat", "sf", "sp", "terra")) %do% {
  # Perform KED for each model
...
# Save the results in appropriate folder
save(ked_interp_tasmax_B1, file = "Projet_1/Interpolation_finale/Etude_de_cas/ked_interp_tasmax_B1.RData")
