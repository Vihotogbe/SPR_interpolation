...
prop_NA_pr <- 0.30
n_spatterns_pr <- 1 #custom_round(n_pr * prop_NA_pr)
spr_interp_pr_B1_2 <- foreach(i = 1:length(n_spatterns_pr), .combine = rbind,
...
save(spr_interp_pr_B1_2, file = "Projet_1/Analyses_finales/Etude_de_cas/spr_interp_pr_B1_2.RData")
...
prop_NA_tasmin <- 0.30
n_spatterns_tasmin <- 2 #custom_round(n_tasmin * prop_NA_tasmin)
spr_interp_tasmin_B1_2 <- foreach(i = 1:length(n_spatterns_tasmin), .combine = rbind,
                            .packages = c("bigmemory", "bigstatsr")) %dopar% {
  # Perform SPR for each n_spatterns
...
# Save the results in appropriate folder
save(spr_interp_tasmin_B1_2, file = "Projet_1/Analyses_finales/Etude_de_cas/spr_interp_tasmin_B1_2.RData")

## Maximum temperature
data_tasmax_test <- data_tasmax_obs[, - test_indexes]
n_tasmax <- min(ncol(data_tasmax_grid), ncol(data_tasmax_test))
prop_NA_tasmax <- 0.30
n_spatterns_tasmax <- 2 #custom_round(n_tasmax * prop_NA_tasmax)
spr_interp_tasmax_B1_2 <- foreach(i = 1:length(n_spatterns_tasmax), .combine = rbind,
                            .packages = c("bigmemory", "bigstatsr")) %dopar% {
  # Perform SPR for each n_spatterns
...
# Save the results in appropriate folder
save(spr_interp_tasmax_B1_2, file = "Projet_1/Analyses_finales/Etude_de_cas/spr_interp_tasmax_B1_2.RData")


#######################################################################################################
################# Summary of the ploted day ###########################################################
# SPR interpolation method
# spr_interp <- function(data_rcm_grid, n_spatterns = 10, data_obs_grid) {
  ## data_rcm_grid is the data from which the spatial patterns are calculated : n x p
  ## n is the number of days (rows) and p is the number of stations (columns)
...
data_obs_grid <- data_tasmax_obs
n_spatterns <- 2 # Number of spatial patterns to calculate
  # Calculating the spatial patterns from RCM DATA
  data_rcm_FBM <- as_FBM(data_rcm_grid)
  pca_rcm <- big_randomSVD(data_rcm_FBM, k = n_spatterns, # ncores = 15,
                 fun.scaling = big_scale(center = TRUE, scale = FALSE))
...
#   return(data_interp_spr) # Return the interpolated : m x p
# }
data_obs_interp[-test_indexes] # This is the interpolated data for the day i
data_tasmax_obs_0[3491, -test_indexes] # This is the interpolated data for the test set
#######################################################################################################
